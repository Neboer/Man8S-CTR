...
def build_mbcontainer(
    ...
    # 如果不是分离模式，显示日志（nerdctl compose up -d 会启动容器）
    if not detach:
...
def _handle_copyfrom_mount(
    ...
    try:
        ...
        mb_logger.debug(
            f"[copyfrom] Temporary container created: {temp_container_name}"
        )
        ...
        try:
            ...
            os.chown(
                entry.source.real_mount_source_path, entry.owner[0], entry.owner[1]
            )
            ...
            # 清理临时容器
            mb_logger.debug(
                f"[copyfrom] Removing temporary container: {temp_container_name}"
            )
            client.remove_container(temp_container_name, safe=True, hide=True)
            mb_logger.debug(
                f"[copyfrom] Temporary container removed: {temp_container_name}"
            )
