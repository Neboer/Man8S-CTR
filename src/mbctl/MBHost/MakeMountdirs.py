...
def prepare_mount_entry(mount_entry: MBContainerMountEntry) -> None:
    ...
    if not mount_entry.file:  # 只创建目录挂载点，跳过文件挂载点。
        ...
        os.chown(mount_entry.source.real_mount_source_path, mount_entry.owner[0], mount_entry.owner[1])
        os.chmod(mount_entry.source.real_mount_source_path, int(mount_entry.perm, 8))
    else:
